...
from common.utils import config_from_env_list, wait_for_server, configure_recollection
from common.data import read_sim_json, client_sim, ClientSimulationManager
from algorithms.fedhaf.model import Net, train, val
...
class Client(fl.client.NumPyClient):
    ...
    def fit(
        ...
        fair_loss = float(loss - noise_var) if not self.abs_fair else float(loss)

        return self.get_parameters({}), len(trainloader.dataset), {"train_loss": fair_loss}
    ...
    def evaluate(
        ...
        valloss, _ = val(self.net, valloader, self.device)
        
        testloader = self.sim_manager.get_testloader(batch_size=self.batch_size)
        testloss, _ = val(self.net, testloader, self.device)
...
def main(
    ...
    recollection_strategy, recollection_size_strategy = configure_recollection(
